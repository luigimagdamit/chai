...
pub fn parse_precedence(parser: &mut Parser, precedence: Precedence) {
    parser.advance();
    println!("DEBUG: parse_precedence - after advance, previous token: {:?}", parser.previous.unwrap());

    if let Some(prefix_fn) = get_rule(parser.previous.unwrap().token_type).prefix {
        println!("DEBUG: Found prefix rule for token type: {:?}", parser.previous.unwrap().token_type);
    ...
    while precedence.to_u32() <= get_rule(parser.current.unwrap_or_else(||panic!("Current token not present for parse_precedence()")).token_type).precedence.to_u32() {
        println!("DEBUG: Checking infix for current token: {:?}, precedence: {}",
                 parser.current.unwrap().token_type,
                 get_rule(parser.current.unwrap().token_type).precedence.to_u32());
        parser.advance();
        if let Some(infix_rule) = get_rule(parser.previous.unwrap().token_type).infix {
            println!("DEBUG: Found infix rule for token type: {:?}", parser.previous.unwrap().token_type);
