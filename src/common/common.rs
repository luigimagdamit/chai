...
// EXPRESSION MODE
pub const PARSE_DECLARATION_MODE: bool = false; // make compiler expect a function main body and not include a mainless helper
pub const PARSE_EXPRESSION_MODE: bool = true && !PARSE_DECLARATION_MODE; // will make it so that top level expressions are printed to output as functions like in LLVM tutorial
