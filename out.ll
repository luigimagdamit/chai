...
entry:
	...
	%40 = add i32 %i_0, 0				; expr_pop
	call void @print_i32(i32 %40)

	; pushing a new string on the stack ...
[31m[CompilerError] Line: 31 -  incompatibe variable assignment types at `var`[0m
	store getelementptr inbounds [4 x i8], [4 x i8]* @str4, i32 0, i32 0 ; llvm_retrieve_static_string, i32* %i		 ; set symbol (symbol.rs)

	%flag = alloca i1
	%42 = icmp ult i32 1, 2
	%43 = add i1 %42, 0				; expr_pop
	call void @print_i1(i1 %43)

	%45 = icmp ugt i32 3, 1
	%46 = add i1 %45, 0				; expr_pop
	call void @print_i1(i1 %46)

	%48 = icmp eq i32 2, 3
	%49 = add i1 %48, 0				; expr_pop
	call void @print_i1(i1 %49)

	%51 = add i1 1, 0				; expr_pop
	call void @print_i1(i1 %51)

	%53 = add i1 0, 0				; expr_pop
	call void @print_i1(i1 %53)

	%seppuku = alloca i8*
	; pushing a new string on the stack ...
	%55 = getelementptr inbounds [21 x i8], [21 x i8]* @str5, i32 0, i32 0 ; llvm_retrieve_static_string		;LLVM Register for String @ ExprCount 55(variable.rs) 				; Printing a string type (expr_pop - register increment by LlvmTempRegister)
	store i8* %55, i8** %seppuku				 ; storing item in a stack variable

	%seppuku_0 = load i8*, i8** %seppuku ; 			 Llvm Load String
	%56 = getelementptr inbounds [21 x i8], [21 x i8]* @str5, i32 0, i32 0 ; llvm_retrieve_static_string		;LLVM Register for String @ ExprCount 56(variable.rs) 				; Printing a string type (expr_pop - register increment by LlvmTempRegister)
	call i32 (i8*, ...) @printf(i8* %56)											; Auto generated by LlvmCallPrint (print.rs)
...
@str5 = private unnamed_addr constant [22 x i8] c"ok shutting down now\0A\00", align 1
@str3 = private unnamed_addr constant [22 x i8] c"this is the variable\0A\00", align 1
@str4 = private unnamed_addr constant [4 x i8] c"24\0A\00", align 1
